...
% seperate paramters to be fitted n0 and constant const
ndata = length(T_K);
in = 0;   ic = 0;
...
all_dev = property - prop_calc;
all_re_dev = all_dev ./ prop_calc;
...
for iproperty = 1:nproperty
    if ncount(iproperty) ~= 0
        iplot = iplot + 1;
        subplot(3,2,iplot)
        plot(xdata(nindex(iproperty,1:ncount(iproperty)),2),all_re_dev(nindex(iproperty,1:ncount(iproperty)))*100,markers{iplot},'markerfacecolor',mycolor(iplot,:),'markeredgecolor',mycolor(iplot,:),'markersize',markersize,'linewidth',linewidth-0.1);
        xlabel('\itp \rm / MPa')
