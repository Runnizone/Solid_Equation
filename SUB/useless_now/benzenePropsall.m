...
function benzenePropsall=benzenePropsall(para,input) 
%*************************************************************************
...
%v = 0.8 * v00                                   %Initial guess for root finder
    ...
    perror=100;
    k=1;
    while (perror > eps) && v>0&&k<=100
       
        %GoSub Evaluate% Subroutine to evaluate p and (dp/dv) at given T and v
        
 % Subroutine to evaluate p and (dp/dv) at given T and v
for j = 1:25
...
H=U + p * v;
Ht=benzenePropsfitH0(para)+92.474*278.674;
deltaH=Ht-(-1544.97);
    ...
    benzenePropsall(2) = KappaT;

    benzenePropsall(3) = KappaS;

    benzenePropsall(4) = Alpha*10^4;

    benzenePropsall(5) = cp;

    benzenePropsall(6) = cv;

    benzenePropsall(7) = Alpha * v / (cv * KappaT);

    benzenePropsall(8) = U;

    benzenePropsall(9) = S;

    benzenePropsall(10) = U - T * S;

    benzenePropsall(11)=(H-deltaH)/1000;

    benzenePropsall(12) = U + p * v - T * S;
